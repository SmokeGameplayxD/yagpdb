// Code generated by SQLBoiler (https://github.com/volatiletech/sqlboiler). DO NOT EDIT.
// This file is meant to be re-generated in place and/or deleted at any time.

package models

import (
	"bytes"
	"reflect"
	"testing"

	"github.com/volatiletech/sqlboiler/boil"
	"github.com/volatiletech/sqlboiler/randomize"
	"github.com/volatiletech/sqlboiler/strmangle"
)

func testCommandsChannelsOverrides(t *testing.T) {
	t.Parallel()

	query := CommandsChannelsOverrides(nil)

	if query.Query == nil {
		t.Error("expected a query, got nothing")
	}
}
func testCommandsChannelsOverridesDelete(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	if err = commandsChannelsOverride.Delete(tx); err != nil {
		t.Error(err)
	}

	count, err := CommandsChannelsOverrides(tx).Count()
	if err != nil {
		t.Error(err)
	}

	if count != 0 {
		t.Error("want zero records, got:", count)
	}
}

func testCommandsChannelsOverridesQueryDeleteAll(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	if err = CommandsChannelsOverrides(tx).DeleteAll(); err != nil {
		t.Error(err)
	}

	count, err := CommandsChannelsOverrides(tx).Count()
	if err != nil {
		t.Error(err)
	}

	if count != 0 {
		t.Error("want zero records, got:", count)
	}
}

func testCommandsChannelsOverridesSliceDeleteAll(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	slice := CommandsChannelsOverrideSlice{commandsChannelsOverride}

	if err = slice.DeleteAll(tx); err != nil {
		t.Error(err)
	}

	count, err := CommandsChannelsOverrides(tx).Count()
	if err != nil {
		t.Error(err)
	}

	if count != 0 {
		t.Error("want zero records, got:", count)
	}
}
func testCommandsChannelsOverridesExists(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	e, err := CommandsChannelsOverrideExists(tx, commandsChannelsOverride.ID)
	if err != nil {
		t.Errorf("Unable to check if CommandsChannelsOverride exists: %s", err)
	}
	if !e {
		t.Errorf("Expected CommandsChannelsOverrideExistsG to return true, but got false.")
	}
}
func testCommandsChannelsOverridesFind(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	commandsChannelsOverrideFound, err := FindCommandsChannelsOverride(tx, commandsChannelsOverride.ID)
	if err != nil {
		t.Error(err)
	}

	if commandsChannelsOverrideFound == nil {
		t.Error("want a record, got nil")
	}
}
func testCommandsChannelsOverridesBind(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	if err = CommandsChannelsOverrides(tx).Bind(commandsChannelsOverride); err != nil {
		t.Error(err)
	}
}

func testCommandsChannelsOverridesOne(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	if x, err := CommandsChannelsOverrides(tx).One(); err != nil {
		t.Error(err)
	} else if x == nil {
		t.Error("expected to get a non nil record")
	}
}

func testCommandsChannelsOverridesAll(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverrideOne := &CommandsChannelsOverride{}
	commandsChannelsOverrideTwo := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverrideOne, commandsChannelsOverrideDBTypes, false, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}
	if err = randomize.Struct(seed, commandsChannelsOverrideTwo, commandsChannelsOverrideDBTypes, false, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverrideOne.Insert(tx); err != nil {
		t.Error(err)
	}
	if err = commandsChannelsOverrideTwo.Insert(tx); err != nil {
		t.Error(err)
	}

	slice, err := CommandsChannelsOverrides(tx).All()
	if err != nil {
		t.Error(err)
	}

	if len(slice) != 2 {
		t.Error("want 2 records, got:", len(slice))
	}
}

func testCommandsChannelsOverridesCount(t *testing.T) {
	t.Parallel()

	var err error
	seed := randomize.NewSeed()
	commandsChannelsOverrideOne := &CommandsChannelsOverride{}
	commandsChannelsOverrideTwo := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverrideOne, commandsChannelsOverrideDBTypes, false, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}
	if err = randomize.Struct(seed, commandsChannelsOverrideTwo, commandsChannelsOverrideDBTypes, false, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverrideOne.Insert(tx); err != nil {
		t.Error(err)
	}
	if err = commandsChannelsOverrideTwo.Insert(tx); err != nil {
		t.Error(err)
	}

	count, err := CommandsChannelsOverrides(tx).Count()
	if err != nil {
		t.Error(err)
	}

	if count != 2 {
		t.Error("want 2 records, got:", count)
	}
}

func testCommandsChannelsOverridesInsert(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	count, err := CommandsChannelsOverrides(tx).Count()
	if err != nil {
		t.Error(err)
	}

	if count != 1 {
		t.Error("want one record, got:", count)
	}
}

func testCommandsChannelsOverridesInsertWhitelist(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx, commandsChannelsOverrideColumnsWithoutDefault...); err != nil {
		t.Error(err)
	}

	count, err := CommandsChannelsOverrides(tx).Count()
	if err != nil {
		t.Error(err)
	}

	if count != 1 {
		t.Error("want one record, got:", count)
	}
}

func testCommandsChannelsOverrideToManyChannelsOverrideCommandsChannelsCMDOverrides(t *testing.T) {
	var err error
	tx := MustTx(boil.Begin())
	defer tx.Rollback()

	var a CommandsChannelsOverride
	var b, c CommandsChannelsCMDOverride

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	if err := a.Insert(tx); err != nil {
		t.Fatal(err)
	}

	randomize.Struct(seed, &b, commandsChannelsCMDOverrideDBTypes, false, commandsChannelsCMDOverrideColumnsWithDefault...)
	randomize.Struct(seed, &c, commandsChannelsCMDOverrideDBTypes, false, commandsChannelsCMDOverrideColumnsWithDefault...)

	b.ChannelsOverridesID = a.ID
	c.ChannelsOverridesID = a.ID
	if err = b.Insert(tx); err != nil {
		t.Fatal(err)
	}
	if err = c.Insert(tx); err != nil {
		t.Fatal(err)
	}

	commandsChannelsCMDOverride, err := a.ChannelsOverrideCommandsChannelsCMDOverrides(tx).All()
	if err != nil {
		t.Fatal(err)
	}

	bFound, cFound := false, false
	for _, v := range commandsChannelsCMDOverride {
		if v.ChannelsOverridesID == b.ChannelsOverridesID {
			bFound = true
		}
		if v.ChannelsOverridesID == c.ChannelsOverridesID {
			cFound = true
		}
	}

	if !bFound {
		t.Error("expected to find b")
	}
	if !cFound {
		t.Error("expected to find c")
	}

	slice := CommandsChannelsOverrideSlice{&a}
	if err = a.L.LoadChannelsOverrideCommandsChannelsCMDOverrides(tx, false, (*[]*CommandsChannelsOverride)(&slice)); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ChannelsOverrideCommandsChannelsCMDOverrides); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ChannelsOverrideCommandsChannelsCMDOverrides = nil
	if err = a.L.LoadChannelsOverrideCommandsChannelsCMDOverrides(tx, true, &a); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ChannelsOverrideCommandsChannelsCMDOverrides); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	if t.Failed() {
		t.Logf("%#v", commandsChannelsCMDOverride)
	}
}

func testCommandsChannelsOverrideToManyAddOpChannelsOverrideCommandsChannelsCMDOverrides(t *testing.T) {
	var err error

	tx := MustTx(boil.Begin())
	defer tx.Rollback()

	var a CommandsChannelsOverride
	var b, c, d, e CommandsChannelsCMDOverride

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, commandsChannelsOverrideDBTypes, false, strmangle.SetComplement(commandsChannelsOverridePrimaryKeyColumns, commandsChannelsOverrideColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*CommandsChannelsCMDOverride{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, commandsChannelsCMDOverrideDBTypes, false, strmangle.SetComplement(commandsChannelsCMDOverridePrimaryKeyColumns, commandsChannelsCMDOverrideColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}

	if err := a.Insert(tx); err != nil {
		t.Fatal(err)
	}
	if err = b.Insert(tx); err != nil {
		t.Fatal(err)
	}
	if err = c.Insert(tx); err != nil {
		t.Fatal(err)
	}

	foreignersSplitByInsertion := [][]*CommandsChannelsCMDOverride{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddChannelsOverrideCommandsChannelsCMDOverrides(tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}

		first := x[0]
		second := x[1]

		if a.ID != first.ChannelsOverridesID {
			t.Error("foreign key was wrong value", a.ID, first.ChannelsOverridesID)
		}
		if a.ID != second.ChannelsOverridesID {
			t.Error("foreign key was wrong value", a.ID, second.ChannelsOverridesID)
		}

		if first.R.ChannelsOverride != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.ChannelsOverride != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.ChannelsOverrideCommandsChannelsCMDOverrides[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ChannelsOverrideCommandsChannelsCMDOverrides[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ChannelsOverrideCommandsChannelsCMDOverrides(tx).Count()
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
			t.Error("want", want, "got", count)
		}
	}
}

func testCommandsChannelsOverridesReload(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	if err = commandsChannelsOverride.Reload(tx); err != nil {
		t.Error(err)
	}
}

func testCommandsChannelsOverridesReloadAll(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	slice := CommandsChannelsOverrideSlice{commandsChannelsOverride}

	if err = slice.ReloadAll(tx); err != nil {
		t.Error(err)
	}
}
func testCommandsChannelsOverridesSelect(t *testing.T) {
	t.Parallel()

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	slice, err := CommandsChannelsOverrides(tx).All()
	if err != nil {
		t.Error(err)
	}

	if len(slice) != 1 {
		t.Error("want one record, got:", len(slice))
	}
}

var (
	commandsChannelsOverrideDBTypes = map[string]string{`AutodeleteNewCMDS`: `boolean`, `ChannelIds`: `ARRAYbigint`, `DisableNewCMDS`: `boolean`, `GuildID`: `bigint`, `HideCMDDisabledMSG`: `boolean`, `ID`: `bigint`, `IsGlobal`: `boolean`, `OverrideEnabled`: `boolean`, `ReqRoleNewCMDS`: `ARRAYbigint`}
	_                               = bytes.MinRead
)

func testCommandsChannelsOverridesUpdate(t *testing.T) {
	t.Parallel()

	if len(commandsChannelsOverrideColumns) == len(commandsChannelsOverridePrimaryKeyColumns) {
		t.Skip("Skipping table with only primary key columns")
	}

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	count, err := CommandsChannelsOverrides(tx).Count()
	if err != nil {
		t.Error(err)
	}

	if count != 1 {
		t.Error("want one record, got:", count)
	}

	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	if err = commandsChannelsOverride.Update(tx); err != nil {
		t.Error(err)
	}
}

func testCommandsChannelsOverridesSliceUpdateAll(t *testing.T) {
	t.Parallel()

	if len(commandsChannelsOverrideColumns) == len(commandsChannelsOverridePrimaryKeyColumns) {
		t.Skip("Skipping table with only primary key columns")
	}

	seed := randomize.NewSeed()
	var err error
	commandsChannelsOverride := &CommandsChannelsOverride{}
	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverrideColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Insert(tx); err != nil {
		t.Error(err)
	}

	count, err := CommandsChannelsOverrides(tx).Count()
	if err != nil {
		t.Error(err)
	}

	if count != 1 {
		t.Error("want one record, got:", count)
	}

	if err = randomize.Struct(seed, commandsChannelsOverride, commandsChannelsOverrideDBTypes, true, commandsChannelsOverridePrimaryKeyColumns...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	// Remove Primary keys and unique columns from what we plan to update
	var fields []string
	if strmangle.StringSliceMatch(commandsChannelsOverrideColumns, commandsChannelsOverridePrimaryKeyColumns) {
		fields = commandsChannelsOverrideColumns
	} else {
		fields = strmangle.SetComplement(
			commandsChannelsOverrideColumns,
			commandsChannelsOverridePrimaryKeyColumns,
		)
	}

	value := reflect.Indirect(reflect.ValueOf(commandsChannelsOverride))
	updateMap := M{}
	for _, col := range fields {
		updateMap[col] = value.FieldByName(strmangle.TitleCase(col)).Interface()
	}

	slice := CommandsChannelsOverrideSlice{commandsChannelsOverride}
	if err = slice.UpdateAll(tx, updateMap); err != nil {
		t.Error(err)
	}
}
func testCommandsChannelsOverridesUpsert(t *testing.T) {
	t.Parallel()

	if len(commandsChannelsOverrideColumns) == len(commandsChannelsOverridePrimaryKeyColumns) {
		t.Skip("Skipping table with only primary key columns")
	}

	seed := randomize.NewSeed()
	var err error
	// Attempt the INSERT side of an UPSERT
	commandsChannelsOverride := CommandsChannelsOverride{}
	if err = randomize.Struct(seed, &commandsChannelsOverride, commandsChannelsOverrideDBTypes, true); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = commandsChannelsOverride.Upsert(tx, false, nil, nil); err != nil {
		t.Errorf("Unable to upsert CommandsChannelsOverride: %s", err)
	}

	count, err := CommandsChannelsOverrides(tx).Count()
	if err != nil {
		t.Error(err)
	}
	if count != 1 {
		t.Error("want one record, got:", count)
	}

	// Attempt the UPDATE side of an UPSERT
	if err = randomize.Struct(seed, &commandsChannelsOverride, commandsChannelsOverrideDBTypes, false, commandsChannelsOverridePrimaryKeyColumns...); err != nil {
		t.Errorf("Unable to randomize CommandsChannelsOverride struct: %s", err)
	}

	if err = commandsChannelsOverride.Upsert(tx, true, nil, nil); err != nil {
		t.Errorf("Unable to upsert CommandsChannelsOverride: %s", err)
	}

	count, err = CommandsChannelsOverrides(tx).Count()
	if err != nil {
		t.Error(err)
	}
	if count != 1 {
		t.Error("want one record, got:", count)
	}
}
